...
- **Domain:** Aptos Wallet.
- **Key Libraries:** `@mysten/bcs`, `@noble/curves`, `@noble/hashes`, `micro-key-producer` (SLIP-0010); custom REST RPC / BCS tx encoding (`src/aptos-rpc.js`, `src/transaction.js`) instead of `@aptos-labs/ts-sdk` for Bare compatibility.
- **Standards:** SLIP-0010 Ed25519 (`m/44'/637'`, all path segments hardened).
- **Features:** Native APT transfers, fungible asset (FA) transfers, simulate-for-gas quoting.
